...
def Est(x):
    ...
    if len(op.args)==1:
        ...
        c, s = op.args[0], op.args[1]
     
    return c, s 


class NumberBoson(BosonicOperator, Number):
    
    op_symbol = 'n'

    def _dagger_(self):
        return NumberBoson(self.state)
    ...
    def __repr__(self):
        return "NumberBoson(%s)" % self.state

    def _latex(self, printer):
        return "n_{%s}" % self.state.name
...
class AnnihilateBoson(BosonicOperator, Annihilator):
    ...
    def apply(self, state):
        ...
        if not self.is_symbolic and isinstance(state, FockStateKet):
            ...
            amp = state[element]
            return C*amp*state.down(element)
        else:
            return Mul(self, state)
